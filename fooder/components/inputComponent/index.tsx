...
export const InputComponent = ({ value, onChange, type, className, id, required, placeholder, onKeyUp }: Props) => {
    return (
        <input type={type} id={id} value={value} onChange={e => onChange(e.target.value)} className={`text-sm w-full rounded-md p-2 bg-slate-50 border border-secondary focus:border-primary focus:outline-none ${className}`} required={required ? required : false} placeholder={placeholder || ""} onKeyUp={e => {
    ...
    return (
        <div className="w-full flex flex-col gap-1 my-2">
            ...
            <div className="w-full flex items-center gap-1 bg-white border-slate-500 rounded-md border">
                ...
                <input type={type} id={id} value={value} onChange={e => onChange(e.target.value)} className={`text-sm w-full rounded-r-md p-2 bg-white focus:outline-none ${className}`} required={required ? required : false} placeholder={placeholder || ""} readOnly={readOnly ? readOnly : false} onKeyUp={e => {
                    if (onKeyUp) onKeyUp(e)
                }} />
            </div>
        </div>
