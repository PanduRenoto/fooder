...
import { ButtonSuccess, ButtonDanger, ButtonInfo } from "@/components/button";
        ...
        <div>
            ...
            <Modal isShow={isShow} onClose={state => setIsShow(state)}>
                <form onSubmit={handleSubmit}>
                    ...
                    {/* modal body */}
                    <div className="p-5">
                        <InputGroupComponent id={`name`} type="text" value={menu.name} onChange={val => setMenu({ ...menu, name: val })} required={true} label="Name" onKeyUp={() => { }} />
                        <InputGroupComponent id={`price`} type="number" value={menu.price.toString()} onChange={val => setMenu({ ...menu, price: Number(val) })} required={true} label="Price" onKeyUp={() => { }} />
                        <InputGroupComponent id={`description`} type="text" value={menu.description} onChange={val => setMenu({ ...menu, description: val })} required={true} label="Description" onKeyUp={() => { }} />
                        ...
                        <FileInput acceptTypes={["application/pdf", "image/png", "image/jpeg", "image/jpg"]} id="profile_picture" label="Unggah Foto (Max 2MB, PDF/JPG/JPEG/PNG)" onChange={f => setFile(f)} required={false} />
                    </div>
                    {/* modal footer */}
